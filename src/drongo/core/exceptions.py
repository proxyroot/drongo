...
most important thing drongo has to get right is the status code; the body simply
carries a helpful message. :class:`DrongoHttpError` models that envelope and knows
...
# - just the codes drongo actually emits.
_STATUS_BY_CODE = {
...
class DrongoHttpError(Exception):
...
def bad_request(message: str, *, reason: str = "invalid") -> DrongoHttpError:
    return DrongoHttpError(400, message, reason=reason)


def not_found(message: str, *, reason: str = "notFound") -> DrongoHttpError:
    return DrongoHttpError(404, message, reason=reason)


def already_exists(message: str, *, reason: str = "conflict") -> DrongoHttpError:
    return DrongoHttpError(409, message, reason=reason)


def precondition_failed(
    message: str, *, reason: str = "conditionNotMet"
) -> DrongoHttpError:
    return DrongoHttpError(412, message, reason=reason)


def not_implemented(message: str) -> DrongoHttpError:
    return DrongoHttpError(501, message, reason="notImplemented")
