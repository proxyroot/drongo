...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--version", action="version", version=f"gato {gato.__version__}"
    )
    ...
    sub.add_parser("services", help="List the GCP services gato can mock.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "server":
        from gato.server import run

        run(host=args.host, port=args.port)
        return 0

    if args.command == "services":
        import gato.services  # noqa: F401  (register services)
        from gato.core.registry import iter_services
